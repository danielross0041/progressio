						...
						<div class="row align-items-center">
							...
							<div class="col-md-3">
								<div class="lesson_btn">
									<a target="_blank" href="{{route('tutor.endsession',[$result])}}">Report</a>
									<a href="{{route('tutor.chat',[$result])}}">Chat</a>
									@if($result->status==0)
										<a href="{{route('customer.attend',[$result])}}">Start</a>
										<a onclick="cancelBooking({{$result->id}})" class="btn-danger" href="javascript:void(0)">Cannot Attend</a>
									@endif
								</div>
							</div>
						</div>
...
function cancelBooking(bookingid){
	...
	.then((willDelete) => {
	  if (willDelete) {
		window.location.href='{{route('tutor.booking.cancel')}}/'+bookingid
	  } else {
		swal.close();
	  }
	});
