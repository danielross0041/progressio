...
class BookingController extends Controller
{
    ...
    public function booktutor(){
		$pupils = User::where('user_type',2)->where('school_id',Auth::user()->school_id)->orderBy('id','asc')->get();
        ...
        return view('customer.teacher.tutor.book')->with('title','Book tutor')
	...
	public function provideDetails(){
		...
		if(Session::has('cart')){
			...
			if(count($cart)>0){
				...
				return view('customer.teacher.tutor.bookingDetails')->with('title','Provide Booking Request Details')
				->with('data',$data);
			}
		}
	}
	...
	public function report(bookings $booking){
		...
		return view('customer.teacher.report')->with(compact('booking','title'));
	}
}
