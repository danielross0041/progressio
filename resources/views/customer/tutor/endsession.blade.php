    ...
    <div class="row">
      <div class="col-md-12 pt-5 pb-5">
        <div class="dashborad_main_sec">
		  ...
		  <form id="finalizeBookingForm" method="POST" action="{{route('tutor.updatereport',[$booking])}}">
			@csrf
				...
				<div class="row mb-3">
					<div class="col-md-6">
						<p><b>Lesson Title: </b><input class="form-control" type="text" name="lesson_title[{{$dat->id}}]" value="{{$dat->lesson_title}}" /></p>
						{{Helper::errorField('lesson_title.'.$dat->id,$errors)}}
						<p><b>What we covered: </b><input class="form-control" type="text" name="what_covered[{{$dat->id}}]"  value="{{$dat->what_covered}}" /></p>
						{{Helper::errorField('what_covered.'.$dat->id,$errors)}}
						<p><b>What the pupil did well: </b><input class="form-control" type="text" name="pupil_did_well[{{$dat->id}}]"  value="{{$dat->pupil_did_well}}" /></p>
						{{Helper::errorField('pupil_did_well.'.$dat->id,$errors)}}
						<p><b>What we will cover next time: </b><input class="form-control" type="text" name="what_cover_nextime[{{$dat->id}}]" value="{{$dat->what_cover_nextime}}" /></p>
						{{Helper::errorField('what_cover_nextime.'.$dat->id,$errors)}}
					</div>
					<div class="col-md-6">
					<p><b>What the pupil struggled with: </b></p>
					<textarea name="pupil_struggle[{{$dat->id}}]" class="form-control" rows="12" cols="6" placeholder="">{{$dat->pupil_struggle}}</textarea>
