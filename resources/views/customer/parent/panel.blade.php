						...
						<div class="row align-items-center">
							...
							<div class="col-md-3">
								<div class="lesson_btn">
									<a target="_blank" href="{{route('parent.report',[$result])}}">Report</a>
									@if($result->status==2)
										<a href="{{route('parent.markcomplete',[$result])}}">Mark Complete</a>
									@endif
									@if($result->status==0)
...
function cancelBooking(bookingid){
	...
	.then((willDelete) => {
	  if (willDelete) {
		window.location.href='{{route('parent.booking.cancel')}}/'+bookingid
	  } else {
		swal.close();
	  }
	});
