...
async function fastCRUDForm(ytableObj,_selectedData=undefined,viewmode='create/edit'){
	...
	for(i=0;i<formLength;i++){
		...
		else if(formColumns[i].type=='file'){
			document.getElementById('ytable-FastCRUDFields').innerHTML+=await createFileArea(formColumns[i],val,imagesdata);
		}
...
var createFileArea = async ({name,column,_table},value,imagesdata)=>{
	/*value variable is of no use now as we have added promises and feteched all images at once*/
	var image = await getimage(imagesdata,_table,1);
	var _src='';
	var _hasImage=false;
	...
	var _videoLink = '';
	if(_src!=''){
		_videoLink='<a download href="'+_src+'" target="_blank" data-toggle="tooltip" title="Preview/Download File" class="btn btn-dark btn--icon-text"><i class="zmdi zmdi-eye"></i> File</a>'
	}
	return '<div class="form-group"><label for="'+column+'">\
        '+name+'\
    </label><div class="col-md-4 no-padding-left">\
